...
function build_fcrepo4 {
  if [ "$BUILDER" == "skip-build" ]; then
    packer validate -var-file=vars.json fcrepo4.json
  else
    packer validate $BUILDER -var-file=vars.json fcrepo4.json
  fi
  ...
  if [ -z "$CONTINUOUS_INTEGRATION" ]; then
    humanize-repo-url
    if [ ! -z "$GIT_REPO_URL" ]; then REPO_URL_VAR="--var packer_fcrepo4_repo=$GIT_REPO_URL"; fi
    ...
    if [ "$BUILDER" != "skip-build" ]; then
      PACKER_LOG=$PACKER_LOG_VALUE packer build $BUILDER $REPO_URL_VAR -var-file=vars.json fcrepo4.json
      ...
      if [[ "$BUILDER" == *docker* ]]; then
        ...
        extract_from_json "FCREPO4_VERSION" "packer_fcrepo4_version" "fcrepo4"

        # Write out the simple fig.yml file
        if [ ! -z "$DOCKER_USER" ]; then
          echo "fcrepo4:" > fig.yml
          echo "  image: $DOCKER_USER/packer-fcrepo4:$FCREPO4_VERSION" >> fig.yml
...
if hash strip-json-comments 2>/dev/null; then
  strip-json-comments packer-fcrepo4.json > fcrepo4.json
  build_fcrepo4
elif [ -f fcrepo4.json ]; then
  build_fcrepo4
else
  echo "  strip-json-comments needs to be installed to generate the fcrepo4.json file"
